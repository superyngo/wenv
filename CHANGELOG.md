...
### Fixed
- 2026-06-23 — PowerShell source entries now derive their display name from both `/` and `\` path separators, so Windows-style paths like `.\aliases.ps1` resolve to `aliases` on any host (previously mis-stemmed to `.\aliases` on Unix).
