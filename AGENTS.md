...
### AI Agent 发布行为 (Release Workflow for AI Agents)

当用户要求发布新版本时，AI Agent **必须**按以下步骤执行：

| 步骤 | 行动 | 说明 |
|------|------|------|
| **1. 询问新版号** | 向用户确认目标版本号 | 根据变更内容建议 MAJOR/MINOR/PATCH，等待用户确认 |
| **2. 收集更新资讯** | 执行 `git log <last-tag>..HEAD` | 整理自上次发布以来的所有变更，分类至 Changelog 区块 |
| **3. 更新文件** | 更新 `README.md`、`CHANGELOG.md`、`Cargo.toml` | 同步版本号与功能描述 |
| **4. 打 Tag** | 创建 annotated tag 并撰写 release notes | 使用 `git tag -a vX.Y.Z -m "..."` |
| **5. 发布更新** | 推送至 GitHub 并发布到 crates.io | 执行 `git push` + `cargo publish` |
...
│ Step 1: 询问新版号                                               │
├─────────────────────────────────────────────────────────────────┤
│ • 执行 git tag --sort=-v:refname | head -5 查看最近版本          │
│ • 执行 git log <last-tag>..HEAD --oneline 查看变更               │
│ • 根据变更性质建议版本号（SemVer）                                │
│ • **必须等待用户确认版本号后才能继续**                            │
└─────────────────────────────────────────────────────────────────┘
                              ↓
...
│ Step 2: 收集更新资讯                                             │
├─────────────────────────────────────────────────────────────────┤
│ • 分析 commits 并分类：Added / Changed / Fixed / Removed 等       │
│ • 识别 Breaking Changes                                          │
│ • 整理成用户可读的 release notes 草稿                             │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ Step 3: 更新文件                                                 │
├─────────────────────────────────────────────────────────────────┤
│ • CHANGELOG.md: 将 [Unreleased] 移至新版本区块                    │
│ • Cargo.toml: 更新 version 字段                                  │
│ • README.md: 更新功能描述（如有重大变更）                         │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ Step 4: 打 Tag 含 Release Notes                                  │
├─────────────────────────────────────────────────────────────────┤
│ • git add -A && git commit -m "chore: release vX.Y.Z"            │
│ • git tag -a vX.Y.Z -m "Release vX.Y.Z\n\n<release notes>"       │
│ • Tag message 应包含主要变更摘要                                  │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ Step 5: 发布更新                                                 │
...
│ • git push origin vX.Y.Z  (触发 GitHub Actions 自动构建)         │
│ • cargo publish  (发布到 crates.io)                              │
│ • 验证 GitHub Release 页面与 crates.io 发布状态                   │
└─────────────────────────────────────────────────────────────────┘
```

**关键约束：**
- Step 1 **必须阻断等待用户确认**，禁止自行决定版本号
- 所有文件更新完成后才能进行 commit
- Tag 必须使用 annotated tag (`git tag -a`)
- 发布到 crates.io 前确保 `cargo test` 与 `cargo clippy` 通过
